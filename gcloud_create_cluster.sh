...
create_cluster() {
	...
	echo "gcloud compute instances delete descheduler-$master_uuid --zone=us-east1-b --quiet" > delete_cluster.sh
       
	gcloud compute instances create descheduler-$node1_uuid --image="ubuntu-1704-zesty-v20171011" --image-project="ubuntu-os-cloud" --zone=us-east1-b
	echo "gcloud compute instances delete descheduler-$node1_uuid --zone=us-east1-b --quiet" >> delete_cluster.sh

	gcloud compute instances create descheduler-$node2_uuid --image="ubuntu-1704-zesty-v20171011" --image-project="ubuntu-os-cloud" --zone=us-east1-b
	echo "gcloud compute instances delete descheduler-$node2_uuid --zone=us-east1-b --quiet" >> delete_cluster.sh
...
transfer_install_files() {
	...
	gcloud compute copy-files  kubeadm_preinstall.sh descheduler-$node1_uuid:/tmp --zone=us-east1-b
	gcloud compute copy-files  kubeadm_preinstall.sh descheduler-$node2_uuid:/tmp --zone=us-east1-b
}
...
install_kube() {
	...
	gcloud compute ssh --dry-run descheduler-$node1_uuid --command "sudo apt-get update; sudo apt-get install -y docker.io" --zone=us-east1-b
	gcloud compute ssh --dry-run descheduler-$node2_uuid --command "sudo apt-get update; sudo apt-get install -y docker.io" --zone=us-east1-b
	...
	gcloud compute copy-files descheduler-$master_uuid:/tmp/admin.conf /tmp/admin.conf --zone=us-east1-b
	...
	gcloud compute copy-files kubeadm_join.sh descheduler-$node1_uuid:/tmp --zone=us-east1-b
	...
	gcloud compute copy-files kubeadm_join.sh descheduler-$node2_uuid:/tmp --zone=us-east1-b
