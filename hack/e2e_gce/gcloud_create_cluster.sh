...
generate_kubeadm_instance_files() {
	...
	echo "kubeadm init --kubernetes-version=${kube_version} --apiserver-advertise-address=${master_public_ip}" --skip-preflight-checks> $E2E_GCE_HOME/kubeadm_install.sh
}
...
install_kube() {
	...
	gcloud compute copy-files $E2E_GCE_HOME/kubeadm_join.sh descheduler-$node1_uuid:/tmp --zone=us-east1-b
	...
	gcloud compute copy-files $E2E_GCE_HOME/kubeadm_join.sh descheduler-$node2_uuid:/tmp --zone=us-east1-b
